...
/*
 * Preconditions:	The char ascii values being sent are int values for A-Z a space char
 * Postconditions:	Returns the 0-26 value of characters
 * Return:		int value 0-26
 */
int convertEncInt(int msgInt){
	 	//if the acii values are equal to between 65 and 90 return their
	 	//value minus 65 for formated 0-25 int
 		if( (msgInt >= 65) && (msgInt) <= 90){
 			return (msgInt-65);
 		}else{
...
* Postconditions:	The function encrypts message to ciphertext
* Return:	a pointer to a string of encrypted characters
*/
char* encrypt(char* key,char* msg){
	 //initialize variable for use and initialize and allocate memory for encrypted message
	 int keyInt, msgInt, encryptedInt, n, msgSize = strlen(msg)+1;
	 char* encryptedMsg = (char*) malloc(msgSize*sizeof(char));
	 //loop through each char in message
	 for(n = 0; n < msgSize-1; n++){
		 //convert message and key values to their values from 1-26
		 msgInt = convertEncInt(msg[n]);
		 keyInt = convertEncInt(key[n]);

		 //calculate ciphertext int value
		 encryptedInt = (msgInt + keyInt) % 27;

		 //convert ciphertext int values back to chars and store in msg
		 if(encryptedInt == 26){
			 encryptedMsg[n] = ' ';
		 }else{
			 encryptedMsg[n] = (char) (encryptedInt + 65);
		 }
 ...
 * Postconditions:	sends an encryption identifier to the client to identify whether or not it is otp_enc_d or otp_dec_d.
 * Return:		none
 */
void sendAck(int fd){
		...
		switch(spawnpid){
			...
			case 0:
				...
				if(badContents(key) || badContents(message)){
					fprintf(stderr,"SERVER ERROR: Bad content in file\n");
					free(message);
					free(key);
					exit(1);
				}

				//create an encrypted message from the message message
				char* encryptedMsg = encrypt(key, message);



				//send the client the encrpted data
				writeSocket(encryptedMsg, establishedConnectionFD);
